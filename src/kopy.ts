...
function kopy<T>(
  ...
  if (isArray(data)) {
    ...
    if (isDeep) {
      ...
      for (let i = 0; i < len; i++) {
        result[i] = kopy((data as unknown[])[i], { deep: isDeep, cache, maxDepth }, currentDepth + 1);
      }
    } else {
      // 浅克隆直接复制数组元素
  ...
  if (isDate(data)) {
    ...
    result = isDeep
      ? new Set([...set].map(item => 
          kopy(item, { deep: isDeep, cache, maxDepth }, currentDepth + 1)))
      : new Set([...set]);
  } else if (isMap(data)) {
    // Map对象需要同时克隆键和值
    ...
    result = isDeep
      ? new Map([...map].map(([k, v]) => [
          kopy(k, { deep: isDeep, cache, maxDepth }, currentDepth + 1),
          kopy(v, { deep: isDeep, cache, maxDepth }, currentDepth + 1)
        ]))
      : new Map([...map]);
